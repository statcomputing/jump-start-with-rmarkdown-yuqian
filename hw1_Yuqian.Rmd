...
will also be calculated as references. Section \@ref(sec:figure) will give box
plots of the bias, after repeating each case 100 times. A brief demonstration
of a code chunk is given in Section \@ref(sec:code). In the end, summary and
...
\begin{align}
    \Phi (t) = \int_\infty ^t {\frac{1}{{2\pi }}} {e^{ - {y^2}/2}}dy
    (\#eq:integral)
\end{align}
...
\begin{align}
   ...
   (\#eq:final)
\end{align}
where $X_i$'s are iid $N(0,1)$ variables. Experiment with the approximation at
...
where 
\begin{align}
   h(y) = \left\{ {\begin{array}{*{20}{c}} {{1 \mathord{\left/
   {\vphantom {1 {\sqrt {2\pi } }}} \right.
   \kern-\nulldelimiterspace} {\sqrt {2\pi } }}}&{y \le t}\\0&{y > t}
   \end{array}} \right.
\end{align} and $g(y) = \frac{1}{{\sqrt {2\pi } }}{e^{ - {y^2}/2}}$.
Based on Monte Carlo method,


Step 1: $h(y) = \left\{ {\begin{array}{*{20}{c}}
{{1 \mathord{\left/
 {\vphantom {1 {\sqrt {2\pi } }}} \right.
 \kern-\nulldelimiterspace} {\sqrt {2\pi } }}}&{y \le t}\\
0&{y > t}
\end{array}} \right.$


Step 2: $g(y) = \frac{1}{{\sqrt {2\pi } }}{e^{ - {y^2}/2}}$
...
Step 4: $\hat \Phi (t) = C{\rm E}[h(x)] \approx \frac{C}{n}\sum\limits_{i = 1}^n {h({x_i})}  = \frac{1}{n}\sum\limits_{i = 1}^n {I({X_i} \le t)} $ 
