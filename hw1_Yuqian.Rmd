...
`Step 3`: Draw $N$ independent and identically distributed random samples from $p(y)$.


`Step 4`: Evaluate
$\hat \Phi (t) = C{\rm E}[h(x)] \approx \frac{C}{n}\sum\limits_{i = 1}^n {h({x_i})}  = \frac{1}{n}\sum\limits_{i = 1}^n {I({X_i} \le t)} $ 
based on samples from step3.



# Results  {#sec:results}

Experiments with the approximation at $n \in \{ {10^2},{10^3},{10^4}\} $ and 
$t \in \{0.0,0.67,0.84,1.28,1.65,2.32,2.58,3.09,3.72\} $ are  shown in Table
\@ref(tab:1st_table). The true values of integration are also provided for comparison.
